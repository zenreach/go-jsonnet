...
import (
	...
	"github.com/zenreach/go-jsonnet/parser"
)

var errSuperUsedOutsideOfObject = errors.New("super cannot be used outside of object")
...
func analyzeVisit(a ast.Node, inObject bool, vars ast.IdentifierSet) error {
	...
	case *ast.InSuper:
		if !inObject {
			return errorSuperOutsideOfObject
		}
		visitNext(a.Index, inObject, vars, s)
	case *ast.SuperIndex:
		if !inObject {
			return errSuperOutsideOfObject
		}
		visitNext(a.Index, inObject, vars, s)
	case *ast.Index:
		visitNext(a.Target, inObject, vars, s)
